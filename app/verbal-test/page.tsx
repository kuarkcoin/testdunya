...
import { Trophy, Timer, ArrowLeft, RotateCcw, ChevronRight, ChevronLeft, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <main className="min-h-screen bg-[#0f172a] text-slate-100 px-4 py-12 font-sans selection:bg-indigo-500/30">
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        >
          <Link href="/" className="flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-white transition-colors">
          ...
          <div className="flex gap-6 items-center">
            <div className="text-center">
              ...
              <div className="flex items-center gap-2">
                <Timer size={14} className={timeLeft < 60 ? 'text-red-500' : 'text-emerald-400'} />
                <p className={`font-mono font-bold ${timeLeft < 60 ? 'text-red-500 animate-pulse' : 'text-emerald-400'}`}>
                  {mmss(timeLeft)}
                </p>
              </div>
            </div>
            ...
            <div className="text-center">
              <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest">Question</p>
        ...
        {!finished ? (
          <div className="space-y-8">
            ...
            {/* QUESTION AREA - AnimatePresence ile sorular arası geçiş */}
                ...
                <div className="grid gap-4 pt-4">
                  {q.optionsText.map((opt, i) => {
                    ...
                    return (
                      ...
                      >
                        <div className={`w-10 h-10 rounded-full border flex items-center justify-center text-xs font-black transition-colors ${isSelected ? 'bg-indigo-500 border-indigo-400 text-white' : 'border-white/10 bg-white/5 text-slate-500 group-hover:border-white/30'}`}>
          ...
          >
            <div className="space-y-2">
              ...
              <h2 className="text-sm font-black text-indigo-500 uppercase tracking-[0.4em]">Test Completed</h2>
            ...
            <div className="grid grid-cols-2 gap-4 max-w-sm mx-auto">
              <div className="bg-white/5 p-6 rounded-3xl border border-white/5">
                <p className="text-[10px] text-slate-500 font-black uppercase mb-2 tracking-widest">Correct</p>
                <p className="text-3xl font-black text-emerald-400">{score.correctCount} / {total}</p>
              </div>
              <div className="bg-white/5 p-6 rounded-3xl border border-white/5">
                <p className="text-[10px] text-slate-500 font-black uppercase mb-2 tracking-widest">Time Spent</p>
                <p className="text-3xl font-black text-indigo-400">{mmss(20 * 60 - timeLeft)}</p>
              </div>
            </div>
