...
export default function Page() {
  ...
  // 3) Oyun seçimi (URL id veya lastGameId veya ilk oyun)
  const byId = useMemo(() => Object.fromEntries(games.map((g) => [g.id, g])) as Record<string, OrderingGame>, [games]);
  ...
  const index = useMemo(() => (games.findIndex((g) => g.id === gameId) >= 0 ? games.findIndex((g) => g.id === gameId) : 0), [games, gameId]);
  ...
  const next = () => {
    if (!games.length) return;
    const newIdx = (index + 1) % games.length;
    setGameId(games[newIdx].id);
  };

  const prev = () => {
    if (!games.length) return;
    const newIdx = (index - 1 + games.length) % games.length;
    setGameId(games[newIdx].id);
  };
      ...
      <div className="min-h-screen bg-white">
        <div className="mx-auto max-w-2xl p-6">
          ...
          <p className="mt-2 text-slate-600">
            {loadErr ?? "public/data/games6/ordering.json bulunamadı veya boş."}
          </p>
