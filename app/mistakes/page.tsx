...
function formatText(text: string) {
  ...
  return parts.map((part, index) => {
    if (part.startsWith('**') && part.endsWith('**')) {
      ...
      return (
        <span key={index} className="bg-indigo-50 text-indigo-700 font-bold px-2 py-0.5 rounded mx-1 border border-indigo-100">
          {content}
        </span>
      );
    }
...
export default function MyMistakesPage() {
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        setMistakes(JSON.parse(raw).reverse());
      } catch (e) {
      ...
      <div className="max-w-3xl mx-auto space-y-6">
          ...
          <div className="flex gap-3">
             ...
             {mistakes.length > 0 && (
               <button 
                 ...
                 className="px-4 py-2 text-sm font-bold text-red-600 bg-red-50 border border-red-100 rounded-lg hover:bg-red-100 flex items-center gap-2"
               >
        ...
        {mistakes.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-slate-300">
            ...
            <Link href="/" className="inline-block mt-4 text-indigo-600 font-bold hover:underline">
                Test Çözmeye Başla
            </Link>
          </div>
        ) : (
          <div className="space-y-8">
            {mistakes.map((q, idx) => (
              <div key={idx} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative group">
                ...
                <div className="text-xs font-bold text-slate-400 uppercase mb-3 bg-slate-100 inline-block px-2 py-1 rounded">
                  {q.testTitle || 'Test Sorusu'}
                </div>
                ...
                <div className="grid gap-2 opacity-90">
                  {q.choices.map((c: any) => {
                    // Doğru cevap ID kontrolü
                    const isCorrect = c.id === q.answer;
                    // Kullanıcının yanlış cevabı
