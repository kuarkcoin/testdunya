...
export default function ColorLogicPage() {
  ...
  const generateTask = useCallback((currentLevel: number) => {
    ...
    setCurrentTask({
      ...
      isFlipped: currentLevel >= 4 && Math.random() > 0.5,
      isGhost: currentLevel >= 7 && Math.random() > 0.6
    });
  }, []);
        ...
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-[3rem] p-8 shadow-2xl relative">
          ...
          <div className="flex justify-between items-center mb-10">
            <div className="space-y-1 text-left">
              <p className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em]">Level</p>
          ...
          {gameState === 'playing' && (
            <div className="space-y-12 min-h-[400px] flex flex-col justify-center">
              <div className="text-center space-y-8">
                <div className="inline-flex items-center gap-2 px-4 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] font-black tracking-[0.3em] text-zinc-400 uppercase">
              ...
              <div className="grid grid-cols-3 gap-4">
                {COLORS.map((color) => (
                  <motion.button
                    ...
                    className="h-20 rounded-2xl border border-zinc-800 bg-zinc-900 flex items-center justify-center group"
                  >
          ...
          {gameState === 'idle' && (
            <div className="text-center py-20">
              <h1 className="text-6xl font-black mb-4 leading-none">COLOR<br/><span className="text-cyan-500 italic">LOGIC</span></h1>
              <p className="text-zinc-500 mb-10 text-sm">Rules change as you level up. Stay focused.</p>
              <button onClick={startGame} className="w-full bg-white text-black font-black py-5 rounded-2xl text-xl">START DRILL</button>
            </div>
          )}

          {gameState === 'gameover' && (
            <div className="text-center py-10">
              <h2 className="text-5xl font-black mb-6 italic">FINISHED</h2>
              <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="bg-zinc-800/50 p-6 rounded-3xl text-left">
                ...
                <div className="bg-zinc-800/50 p-6 rounded-3xl text-left">
                  <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Max Lvl</p>
