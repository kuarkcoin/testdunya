...
// Raven-esque frame (visual-matrix için “IQ testi şablonu” görünümü)
function RavenFrame({ children, label = 'MATRIX' }: { children: React.ReactNode; label?: string }) {
  return (
    <div className="w-full max-w-[360px]">
      <div className="rounded-3xl bg-white border-[6px] border-slate-800 shadow-2xl overflow-hidden">
        {/* üst şerit */}
        <div className="px-4 py-2 bg-slate-900 text-white flex items-center justify-between">
          <span className="text-[11px] font-black tracking-[0.28em] uppercase text-slate-200">{label}</span>
          <span className="text-[10px] font-bold text-slate-300">RAVEN STYLE</span>
        </div>
        {/* içerik alanı */}
        <div className="relative bg-white">
          {/* hafif kılavuz desen */}
          <div className="absolute inset-0 opacity-[0.12] bg-[radial-gradient(circle_at_1px_1px,rgba(0,0,0,0.55)_1px,transparent_0)] [background-size:18px_18px]" />
...
export default function IQTestPage() {
  ...
  // Landing ekranı: start butonu
  const showLanding = !started && !finished;
  ...
  // ✅ SCROLL FIX
  useEffect(() => {
    if (!finished && started && questionAreaRef.current) {
  ...
  }, [safeIdx, finished, started]);

  // ✅ A) Timer Optimizasyonu: tek seferlik interval (timeLeft bağımlılığı yok)
  useEffect(() => {
  ...
  const scorePack = useMemo(() => {
    ...
    // Basit “normal dağılım benzeri” ölçek (oyun içi indeks)
    const MU = 0.55;
    const SIGMA = 0.18;
  ...
  }, [answers, data, started, timeLeft, total, TOTAL_TIME]);
          ...
          {showLanding && (
            <section className="rounded-3xl bg-white/5 border border-white/10 p-6 md:p-8 space-y-6">
              <div className="text-center space-y-3">
                <div className="text-2xl md:text-3xl font-black text-white">Start IQ Challenge</div>
                <p className="text-slate-400 max-w-2xl mx-auto">
                  You have <span className="text-white font-black">{mmss(TOTAL_TIME)}</span>. Timer starts when you press Start.
                </p>
              </div>

              <div className="grid md:grid-cols-3 gap-3">
                <div className="rounded-2xl bg-slate-900/50 border border-white/10 p-4">
                  <div className="text-xs uppercase tracking-widest text-slate-400 font-bold mb-2">Rules</div>
                  <ul className="text-sm text-slate-300 space-y-1">
                    <li>• Each question can be answered once.</li>
                    <li>• You can navigate with Prev/Next.</li>
                    <li>• Finish anytime if you want.</li>
                  </ul>
                </div>
                <div className="rounded-2xl bg-slate-900/50 border border-white/10 p-4">
                  <div className="text-xs uppercase tracking-widest text-slate-400 font-bold mb-2">Scoring</div>
                  <ul className="text-sm text-slate-300 space-y-1">
                    <li>• Mixed domains, weighted end-section.</li>
                    <li>• Time bonus included.</li>
                    <li>• Result is an estimate.</li>
                  </ul>
                </div>
                <div className="rounded-2xl bg-slate-900/50 border border-white/10 p-4">
                  <div className="text-xs uppercase tracking-widest text-slate-400 font-bold mb-2">Disclaimer</div>
                  <p className="text-sm text-slate-300">
                    This score is a <span className="text-white font-black">game-based estimate</span> and does not replace
                    professional clinical IQ tests.
                  </p>
                </div>
              </div>
              ...
              <button
                onClick={startTest}
                className="w-full py-4 rounded-2xl font-black bg-indigo-600 hover:bg-indigo-500 transition-colors shadow-lg shadow-indigo-900/25"
              >
                START TEST
              </button>

              <div className="text-center text-[11px] text-slate-500">
                Tip: Use a larger screen for visual matrix questions.
              </div>
            </section>
          )}
            ...
            <section className="rounded-3xl bg-white/5 border border-white/10 p-6 md:p-8 space-y-6">
              <div className="text-center space-y-3">
                ...
                <div className="text-slate-300">
                  ...
                  <span
                    className={`font-black text-3xl md:text-4xl align-middle ${
                      scorePack.gameIQ >= 145 ? 'text-emerald-300 drop-shadow-[0_0_12px_rgba(16,185,129,0.6)]' : 'text-indigo-200'
                    }`}
                  >
                    {scorePack.gameIQ}
                  </span>
                ...
                {/* B) DISCLAIMER */}
                <div className="max-w-3xl mx-auto text-[12px] text-slate-500 bg-white/5 border border-white/10 rounded-2xl px-4 py-3">
                  <span className="font-black text-slate-300">Disclaimer:</span> This score is a game-based estimate for entertainment and training.
                  It does <span className="font-black text-slate-300">not</span> replace professional clinical IQ tests administered by qualified experts.
                </div>
              </div>
              ...
              <section className="rounded-3xl bg-white/5 border border-white/10 p-6 md:p-8 space-y-6">
                ...
                <div ref={questionAreaRef} className="space-y-6">
                  ...
                  <div className="flex justify-center">
                    {'questionSvg' in q ? (
                      // D) Raven-style frame for visual matrices
                      <RavenFrame label={q.type.replaceAll('-', ' ').toUpperCase()}>
                        <div className="w-[280px] h-[280px] md:w-[300px] md:h-[300px] mx-auto">
                    ...
                    {'optionsText' in q ? (
                      ...
                      q.options.map((opt, i) => (
                        ...
                        >
                          ...
                          <div className="absolute top-2 left-2 text-[10px] font-black bg-slate-900 text-white px-2 py-1 rounded-full shadow">
                            {String.fromCharCode(65 + i)}
                          </div>
                          {answers[q.id]?.selected === i && (
                            <div className="absolute top-2 right-2 bg-indigo-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow-sm">
                              ✓
                            </div>
                          )}
                        </button>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-white/5 mt-6">
                    ...
                    {safeIdx >= total - 1 ? (
                      <button
                        onClick={finishNow}
                        className="px-6 py-3 rounded-2xl font-black bg-emerald-600 hover:bg-emerald-500 transition-colors shadow-lg shadow-emerald-900/20"
                      >
                        Finish ✅
                      </button>
                    ) : (
