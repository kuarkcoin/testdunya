...
// Normal CDF yaklaşımı (Abramowitz–Stegun / hızlı approx) — UI’da percentile göstermiyoruz.
...
export default function IQTestPage() {
  ...
  // ✅ Normal dağılıma yakın skor + ELITE/LEGEND badge + Advanced ağırlık + hız bonusu + Elite Run etiketi
  const scorePack = useMemo(() => {
    ...
    // Elite Run: son 12 advanced bölüm
    ...
    iq += 3; // hard-test küçük taban

    // Elite Run: advanced (son 12) doğruluk >= %80 + testi tamamlamış olmak
    ...
    // UI’da göstermiyoruz ama içeride kalsın:
    const _cdf = normCdf(z);
  ...
  const showConfetti = finished && scorePack.gameIQ >= 145; // ELITE ve üstü
  const confettiPieces = scorePack.gameIQ >= 155 ? 520 : 320; // LEGEND daha “sert” patlasın
          ...
          {finished ? (
            <section className="rounded-3xl bg-white/5 border border-white/10 p-6 md:p-8 space-y-6">
              <div className="text-center space-y-3">
                ...
                <div className="text-sm text-slate-400">
                  Advanced (last {Math.min(12, total)}):{' '}
                  <span className="font-black text-white">
                    {scorePack.advCorrect}/{scorePack.advAnswered}
                  </span>{' '}
                  • Rate: <span className="font-black text-white">{scorePack.advRatePct}%</span>
                </div>

                <div className="text-xs text-slate-500">
                  Scoring uses a normal-distribution mapping (mean≈100, sd≈15) with advanced-weight + small speed bonus.
                  Max score is capped at 160.
                </div>
              </div>
              ...
              <div className="rounded-2xl bg-slate-900/40 border border-white/10 p-4">
                ...
                <div className="mt-3 rounded-xl bg-white/5 border border-white/10 p-3 text-sm text-slate-300">
                  <span className="font-black text-fuchsia-200">🚀 ELITE RUN</span> = Last {Math.min(12, total)} advanced
                  questions: <span className="font-black">≥ 80%</span> correct + test fully completed.
                </div>
              </div>
