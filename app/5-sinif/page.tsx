...
// DOĞRUDAN DERS DOSYALARINDAN IMPORT (index.ts iptal edildi)
...
// Tüm verileri sayfa içinde birleştiriyoruz
const allQuestions5 = {
...
export default function Grade5Page() {
  ...
  // AI STATES (Gemini 2.0 Flash ile çalışır)
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 pt-6 md:pt-10">
              ...
              {subjects.map((s) => (
                <button key={s.id} onClick={() => { setSelectedSubject(s.id); setView('term-select'); }} className={`group relative overflow-hidden rounded-[3rem] bg-gradient-to-br ${s.gradient} p-10 text-left transition-all hover:scale-[1.03] shadow-xl`}>
                  <div className="relative z-10">
                    ...
                    <h2 className="text-4xl font-black">{s.label}</h2>
                    <p className="text-white/90 mt-2 font-bold text-lg">{s.desc}</p>
                    <div className="mt-10 inline-flex items-center gap-3 font-black text-sm bg-black/20 px-5 py-3 rounded-2xl group-hover:bg-black/30 transition-all">Konuları Keşfet ➔</div>
                  </div>
          ...
          <div className="max-w-2xl mx-auto py-12 animate-in zoom-in-95 duration-500">
            <div className="bg-slate-800 border border-white/5 p-12 rounded-[4rem] shadow-2xl">
              ...
              <div className="grid gap-6">
                <button onClick={() => { setSelectedTerm(1); setView('test-select'); }} className="p-10 bg-white/5 border border-white/10 rounded-[2.5rem] hover:bg-indigo-600 transition-all flex justify-between items-center"><span className="text-3xl font-black">1. Dönem</span><span className="text-4xl">🚀</span></button>
                <button onClick={() => { setSelectedTerm(2); setView('test-select'); }} className="p-10 bg-white/5 border border-white/10 rounded-[2.5rem] hover:bg-fuchsia-600 transition-all flex justify-between items-center"><span className="text-3xl font-black">2. Dönem</span><span className="text-4xl">🔥</span></button>
              </div>
            </div>
          </div>
        )}

        {/* 3. TEST SEÇİMİ (1'DEN 15'E) */}
        {view === 'test-select' && (
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
              {Array.from({ length: 15 }, (_, i) => i + 1).map((num) => (
                <button
                  ...
                  className="group relative bg-slate-900 border border-white/10 p-10 rounded-[2.5rem] hover:border-indigo-500 hover:scale-105 transition-all shadow-xl flex flex-col items-center gap-2 overflow-hidden"
                >
          ...
          div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500">
            ...
            <div className="bg-slate-800 border border-white/10 p-12 md:p-16 rounded-[3.5rem] shadow-2xl relative overflow-hidden">
              ...
              <div className="grid gap-5">
                {quizQuestions[currentIdx].options.map((opt: string, i: number) => (
                  <button key={i} onClick={() => setAnswers(prev => ({ ...prev, [currentIdx]: i }))} className={`p-8 rounded-[2rem] border-2 text-left transition-all flex items-center gap-6 ${answers[currentIdx] === i ? 'border-indigo-500 bg-indigo-500/10 text-white' : 'border-white/5 bg-slate-950/50 text-slate-400 hover:border-white/20'}`}>
            ...
            <div className="flex justify-between items-center gap-4 px-4 pb-8">
              ...
              {currentIdx === quizQuestions.length - 1 ? (
                <button onClick={() => { setView('result'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} disabled={answers[currentIdx] === undefined} className="px-16 py-6 bg-emerald-600 hover:bg-emerald-500 text-white rounded-[2rem] font-black text-xl shadow-2xl">BİTİR</button>
              ) : (
                <button onClick={() => { setCurrentIdx(p => p + 1); window.scrollTo({ top: 0, behavior: 'smooth' }); }} disabled={answers[currentIdx] === undefined} className="px-16 py-6 bg-indigo-600 hover:bg-indigo-500 text-white rounded-[2rem] font-black text-xl shadow-2xl">SIRADAKİ ➔</button>
              )}
            </div>
          </div>
        )}
        ...
        {view === 'result' && (
          <div className="max-w-4xl mx-auto space-y-12 animate-in zoom-in-95 duration-700 pb-20">
            <div className="bg-slate-900 border border-white/10 p-12 md:p-20 rounded-[4rem] text-center space-y-12 shadow-2xl relative">
              ...
              <div className="bg-indigo-500/10 border border-indigo-500/30 p-10 rounded-[3.5rem] text-left relative overflow-hidden group">
                ...
                {loadingAi ? (
                  <p className="text-indigo-400 font-black italic flex items-center gap-3">
              ...
              <div className="flex flex-col gap-6 pt-6">
                ...
                <button onClick={() => setView('test-select')} className="py-8 bg-slate-800 text-white rounded-[2.5rem] font-black text-2xl hover:bg-slate-700 transition-all shadow-2xl">DİĞER TESTLER</button>
              </div>
            </div>
