          ...
          <div className="max-w-3xl mx-auto space-y-10 animate-in zoom-in-95 duration-500 pb-20">
            <div className="bg-slate-900/80 border border-white/10 p-12 rounded-[4rem] text-center space-y-10 shadow-2xl backdrop-blur-2xl">
                        ...
                        <div className="flex gap-6">
                          ...
                          <div className="space-y-4 w-full">
                            ...
                            <div className="flex flex-wrap gap-2 text-sm font-bold">
                              ...
                              {!isCorrect && <span className="px-3 py-1 bg-red-500/20 text-red-400 rounded-lg">Senin Cevabın: {answers[i] !== undefined ? q.options[answers[i]] : 'Skipped'}</span>}
                            </div>
              ...
              <div className="flex flex-col gap-4 pt-8">
                <button onClick={() => setView('subject-select')} className="w-full py-6 bg-white text-slate-950 rounded-[2rem] font-black text-xl hover:bg-slate-200 transition-all">START NEW SUBJECT</button>
                <button onClick={() => { setView('quiz'); setAnswers({}); setCurrentIdx(0); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="w-full py-6 bg-slate-800 text-white rounded-[2rem] font-black text-xl hover:bg-slate-700 transition-all">RETAKE TEST</button>
              </div>
            </div>
          </div>
        )}
