...
interface Question {
  ...
  C: string;
  D: string;
  E?: string; // YKS ve KPSS 5 şıklı olduğu için opsiyonel
  correct: string;
  Açıklama: string;
...
export default function DynamicTestPage() {
  ...
  // YENİ: Sadece yanlışları filtreleme durumu
    ...
    <div className="min-h-screen bg-slate-100 pb-20">
      ...
      {showResult && (
        <div className="max-w-4xl mx-auto m-4 bg-white rounded-2xl p-6 shadow-xl border-2 border-indigo-100 text-center animate-in fade-in zoom-in duration-300">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            {/* HATALARIM BUTONU: Sadece yanlış varsa göster */}
            {score.wrong > 0 && (
                <button 
                    onClick={() => setShowMistakesOnly(!showMistakesOnly)}
                    className={`px-6 py-3 rounded-lg font-bold transition border-2 flex items-center justify-center gap-2 ${
                        showMistakesOnly 
                ...
                >
                    {showMistakesOnly ? 'Tüm Soruları Göster' : '👀 Sadece Yanlışlarımı Göster'}
                </button>
            )}
          ...
          {showMistakesOnly && (
            <p className="mt-4 text-sm text-rose-600 font-medium animate-pulse">
                Aşağıda sadece yanlış yaptığınız sorular listelenmektedir.
            </p>
          )}
        </div>
      )}
      ...
      <div className="max-w-4xl mx-auto p-4 space-y-8">
        {questions.map((q, index) => {
          ...
          // FİLTRELEME MANTIĞI:
          // Eğer "Sadece Yanlışlar" modu açıksa VE soru yanlış değilse (doğru veya boşsa), bunu gösterme.
          if (showMistakesOnly && !isWrong) return null;
          ...
          return (
            <div key={index} className={`bg-white rounded-2xl p-6 md:p-8 shadow-sm border-2 transition-colors ${
              ...
              <div className="flex gap-4 mb-6">
                <span className={`font-bold px-3 py-1 rounded-lg h-fit text-sm md:text-base shrink-0 ${
                    showMistakesOnly ? 'bg-rose-100 text-rose-800' : 'bg-indigo-100 text-indigo-800'
                }`}>
                  {index + 1}
                </span>
