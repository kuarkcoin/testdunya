...
export default function LgsTest2Page() {
  ...
  // JSON'dan veri çekme
  useEffect(() => {
    // 👇 BURASI ÖNEMLİ: Test 2 verisini çekiyoruz
    fetch("/data/tests/lgs-test-2.json") 
      .then((res) => {
        if (!res.ok) {
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center p-8">
        <div className="text-center">
          ...
          <p className="text-slate-600">Sorular bulunamadı. Lütfen '/public/data/tests/lgs-test-2.json' dosyasını kontrol et.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-2xl mx-auto bg-white rounded-2xl p-6 shadow-lg">
        ...
        {index === questions.length - 1 && showResult && (
          <div className="text-center mt-10 p-6 bg-slate-50 rounded-2xl border border-slate-200">
            ...
            <p className="text-xl font-bold text-slate-800">Test 2 Tamamlandı!</p>
            <Link
              href="/"
