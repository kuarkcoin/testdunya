...
export type Grade5QuizClientProps = {
  initialSubject?: SubjectId;
  initialTerm?: 1 | 2;
  initialTestNo?: number;
  initialDisplayTestNo?: number;
  autoStart?: boolean;
};

export default function Grade5QuizClient({
  initialSubject = 'matematik',
  initialTerm = 1,
  initialTestNo = 1,
  initialDisplayTestNo = initialTestNo,
  autoStart = false,
}: Grade5QuizClientProps) {
  const ui = {
    container: 'max-w-5xl',
  ...
  const [view, setView] = useState<ViewState>(autoStart ? 'quiz' : 'subject-select');
  const [selectedSubject, setSelectedSubject] = useState<SubjectId>(initialSubject);
  const [selectedTerm, setSelectedTerm] = useState<1 | 2>(initialTerm);
  const [selectedTestNo, setSelectedTestNo] = useState<number>(initialDisplayTestNo);

  const [quizQuestions, setQuizQuestions] = useState<Question5[]>(() => {
    if (!autoStart) return [];
    const termKey = initialTerm === 1 ? 'term1' : 'term2';
    return allQuestions5[initialSubject]?.[termKey]?.[`test${initialTestNo}`] ?? [];
  });
...
const textToSpeak =
  ...
  const getQuestionImageAlt = (questionNo: number) =>
    `5. sınıf ${subjectLabel.toLocaleLowerCase('tr-TR')} ${selectedTestNo}. test ${questionNo}. şekilli soru görseli`;
                ...
                <div className="mb-8 flex flex-col items-center justify-center">
                  ...
                  >
                    ...
                    <Image
                      ...
                      alt={getQuestionImageAlt(currentIdx + 1)}
                      width={900}
                      height={600}
                      loading="lazy"
